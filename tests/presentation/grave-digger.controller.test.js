...
describe('Grave Digger controller - Unit Tests',()=>{
  ...
  test('POST gravedigger/ - should create a new gravedigger', async()=>{
    ...
    expect(body.birth_date).toBe(new Date(validGraveDigger.birthDate +' 00:00:000').toISOString())
  ...
  test('GET graveDiggers/:id - should return the created gravedigger', async()=>{
    await request(app).get(`/gravediggers/${graveDigger.id}`).expect(200,graveDigger)
  });
  test('PUT gravediggers/:id - should update the gravedigger', async()=>{
    const updateGraveDigger ={name:'peter predorson',
    cpf: 87548758996,
    ...
    .expect(200,{
      ...
      birth_date: new Date(updateGraveDigger.birthDate + ' 00:00:000').toISOString(),
